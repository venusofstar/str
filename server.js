...
// 🔥 Generate VOD HLS (one-time, not live)
function generateVOD(ep) {
  ...
  if (fs.existsSync(outputFile)) {
    console.log(`${ep} already processed`);
    return;
  }
  ...
  ffmpeg.on("close", () => {
    console.log(`✅ ${ep} VOD ready`);
  });
}

// 🔥 Process all episodes once
Object.keys(EPISODES).forEach(generateVOD);

// 🔥 Serve VOD
app.use("/vod", express.static(OUTPUT));

// 🔥 M3U Playlist (VOD style)
...
// 🔥 Download MP4 (same as yours but cleaner)
